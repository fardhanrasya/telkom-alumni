...
import { ArrowLeft, Mail, Globe, MapPin, Award, Briefcase, Facebook, Instagram, Linkedin, Twitter } from 'lucide-react';
  ...
  if (error || !alumni) {
    return (
      <div className="container mx-auto py-20 text-center">
        <h1 className="mb-4 text-2xl font-bold text-gray-800">Alumni tidak ditemukan</h1>
        <p className="mb-8 text-gray-600">Silakan kembali ke halaman alumni</p>
        <Link href="/alumni">
          <Button variant="default">Kembali ke Daftar Alumni</Button>
        </Link>
      </div>
    );
  }
    ...
    return (
      <div className="container mx-auto max-w-5xl py-12">
        <div className="mb-8 flex items-center">
          ...
          <h1 className="text-2xl font-bold text-gray-900">Detail Alumni</h1>
        </div>

        <div className="flex flex-col items-center justify-center rounded-lg bg-gray-50 p-10 text-center">
          <h2 className="text-xl font-medium text-gray-700">{error || 'Alumni tidak ditemukan'}</h2>
          <p className="mt-2 text-gray-500">Silakan kembali ke halaman alumni</p>
          <Link href="/alumni" className="mt-4">
            <Button>Kembali ke Daftar Alumni</Button>
          </Link>
        </div>
      </div>
    );
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-white to-gray-50 py-12">
      <div className="container mx-auto grid max-w-5xl gap-10 px-4 md:grid-cols-3">
        {/* Kolom kiri - Foto profil & info kontak */}
          ...
          <div className="sticky top-24 space-y-6">
            ...
            <div className="overflow-hidden rounded-xl bg-white shadow-lg ring-1 ring-gray-100">
              {alumni.profileImageUrl ? (
                ...
                <div className="aspect-[3/4] w-full bg-gradient-to-br from-primary-50 to-primary-100 flex items-center justify-center">
                  <span className="text-primary-600 text-6xl font-bold">{alumni.name.charAt(0)}</span>
                </div>
              )}
            </div>
              ...
              <div className="space-y-4">
                {alumni.email && (
                  <div className="flex items-center gap-3 group">
                    <div className="h-10 w-10 flex-shrink-0 flex items-center justify-center rounded-full bg-primary-50 text-primary-600 transition-colors group-hover:bg-primary-100">
                      <Mail className="h-5 w-5" />
                    </div>
                    <a
                      href={`mailto:${alumni.email}`}
                      className="text-sm font-medium text-gray-700 hover:text-primary transition-colors duration-200 hover:underline"
                    >
                      {alumni.email}
                    </a>
                  </div>
                ...
                {alumni.socialMedia && (
                  <div className="mt-6 space-y-3">
                    <h4 className="text-base font-semibold text-gray-800">Media Sosial</h4>
        ...
        <div className="md:col-span-2">
          ...
          <div className="mb-6 flex items-center">
            ...
            <h2 className="text-3xl font-bold text-gray-900 tracking-tight">{alumni.name}</h2>
          </div>
          ...
          {alumni.achievements && alumni.achievements.length > 0 && (
            <div className="mb-10">
              ...
              <div className="space-y-4">
                {alumni.achievements.map((achievement: Achievement, index: number) => (
                  <div
                    key={index}
                    className="rounded-xl border border-gray-100 bg-white p-5 shadow-sm transition-all hover:shadow-md group"
                  >
          ...
          <div className="mt-12 flex justify-center">
            <Link href="/alumni">
              <Button variant="outline" className="flex items-center gap-2 px-6 py-5 rounded-full text-base transition-all hover:bg-primary-50 hover:text-primary-700 hover:border-primary-200 shadow-sm">
                <ArrowLeft className="h-5 w-5" />
