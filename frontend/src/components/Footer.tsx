      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 gap-8 md:grid-cols-3">
          ...
          <div className="space-y-4">
            ...
            <p className="text-sm text-gray-600">
              Portal Alumni SMK Telkom Jakarta, menghubungkan alumni dari berbagai generasi, 
              berbagi informasi dan membangun jaringan profesional.
            </p>
          ...
          <div>
            ...
            <ul className="mt-4 space-y-2">
              {[
                { name: 'Direktori Alumni', href: '/alumni' },
                { name: 'Acara & Kegiatan', href: '/acara' },
                { name: 'Lowongan Kerja', href: '/karir' },
                { name: 'Berita & Pengumuman', href: '/berita' },
                { name: 'Galeri Foto', href: '/galeri' }
              ].map((link) => (
                <li key={link.name}>
                  <Link href={link.href} className="text-gray-600 hover:text-primary transition-colors duration-200">
                    {link.name}
                  </Link>
                </li>
              ))}
            ...
            <ul className="mt-4 space-y-3">
              {[
                { 
                  ...
                  text: 'Jl. Daan Mogot KM.11, Cengkareng, Jakarta Barat'
                },
                { 
                ...
                { 
                  ...
                  text: 'alumni@smktelkom-jkt.sch.id'
                }
              ].map((contact, index) => (
                <li key={index} className="flex items-start">
                  ...
                  <span className="ml-2 text-gray-600">{contact.text}</span>
                </li>
              ))}
            </ul>
          </div>
        ...
        <div className="mt-12 border-t border-gray-200 pt-8">
          <p className="text-center text-sm text-gray-600">
            &copy; {new Date().getFullYear()} Alumni SMK Telkom Jakarta. Hak Cipta Dilindungi.
          </p>
        </div>
      </div>
    </footer>
